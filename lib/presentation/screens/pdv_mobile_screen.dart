...
class _PdvMobileScreenState extends State<PdvMobileScreen> {
  ...
  void _adicionarProdutosSelecionados(List<ProdutoModel> produtos) {
    ...
    _mostrarSnack('${produtos.length} item(ns) adicionados à venda.');
  }
  ...
  Future<void> _finalizarVenda() async {
    ...
    final double totalPago = formas.fold<double>(
      0,
  ...
  double get _total =>
      _itens.fold<double>(0, (soma, item) => soma + item.subtotal);

  int get _quantidadeItens =>
      _itens.fold<int>(0, (soma, item) => soma + item.quantidade);
  ...
  Widget build(BuildContext context) {
    ...
    final double bottomPadding =
        temItens ? (_acoesRapidasVisiveis ? 218 : 142) : 110;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [_primaryColor, Color(0xFF123B69)],
  ...
  Widget _buildFormasPagamentoAdaptativas() {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        final List<List<_FormaPagamentoMobile>> linhas = _montarLinhasPagamento(
          constraints.maxWidth,
        );

        return Column(
          children: linhas
              .map((List<_FormaPagamentoMobile> linha) {
                return Padding(
                  ...
                  child: Row(
                    children: linha
                        .asMap()
                        .entries
                        .map((MapEntry<int, _FormaPagamentoMobile> entry) {
                    ...
                    : () {
                      setState(() {
                        if (selecionado) {
                          ...
                          if (_editandoVendaNaoLiquidada) {
                            for (final codigo in List<String>.from(
                              _formasSelecionadas,
                            )) {
