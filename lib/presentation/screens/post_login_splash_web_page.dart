...
class _SixSplashBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int index = 0; index < 6; index++) {
      final dx = size.width * (0.12 + index * 0.16) + math.sin(progress * math.pi + index) * 12;
      final dy = size.height * (index.isEven ? 0.20 : 0.72) + math.cos(progress * math.pi + index) * 18;
    ...
    for (double x = -20; x <= size.width + 20; x += 18) {
      final y = baseY + math.sin((x / 96) + progress * math.pi * 2) * (compact ? 10 : 16);
      if (x == -20) {
        wave.moveTo(x, y);
      } else {
        wave.lineTo(x, y);
