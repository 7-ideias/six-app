...
class ServicoDashboardWebPage extends StatefulWidget {
  ...
  @override
  State<ServicoDashboardWebPage> createState() =>
      _ServicoDashboardWebPageState();
}
...
class _ServicoDashboardWebPageState extends State<ServicoDashboardWebPage> {
  ...
  void _reload() =>
      setState(() => _future = _produtoService.buscarDashboardServicos());
    ...
    return Material(
      ...
      child: FutureBuilder<ServicoDashboardModel>(
        ...
        builder: (
          ...
          AsyncSnapshot<ServicoDashboardModel> snapshot,
        ) {
          final Widget child;
          ...
          return Column(
            children: <Widget>[
              SixWebDashboardHeader(
                ...
                title: 'Serviços',
                subtitle:
                    'Resumo executivo do catálogo técnico, preços, garantias e pontos de atenção operacional.',
  ...
  Widget _loadingKpis(bool compact) => GridView.builder(
    ...
    itemBuilder:
        (BuildContext context, int index) => SixWebEntry(
          order: index,
          child: SixWebLoadingBlock(height: 118, highlight: index == 2),
        ),
  );
      ...
      return SingleChildScrollView(
        ...
        child: Column(
          ...
          children: <Widget>[
              ...
              children: <Widget>[
                SixWebEntry(
                  ...
                  child: _serieCard(
                    ...
                    'Organização do catálogo por tipo de serviço técnico.',
                ...
                SixWebEntry(
                  ...
                  child: _serieCard(
                    'Faixa de preço',
                    'Distribuição dos serviços por posicionamento comercial.',
                    data.servicosPorFaixaPreco,
                  ),
                ),
                SixWebEntry(
                  order: 10,
                  child: _serieCard(
                    ...
                    'Garantia e flexibilidade de valor no atendimento.',
                    data.configuracaoOperacional,
                  ),
                ),
              ],
            ),
            ...
            SixWebEntry(
              order: 13,
              child: _strategic(data.servicosEstrategicos),
            ),
          ],
        ),
      );
  ...
  Widget _kpis(ServicoDashboardModel data, bool compact) {
    final List<_Kpi> items = <_Kpi>[
      _Kpi(
        ...
        data.totalServicos.toDouble(),
        _whole,
      ),
      _Kpi(
        Icons.verified_outlined,
        'Serviços ativos',
        data.servicosAtivos.toDouble(),
        _whole,
      ),
      _Kpi(
        Icons.sell_outlined,
      ...
      _Kpi(
        ...
        'Sem preço',
        data.servicosSemPreco.toDouble(),
        _whole,
      ),
      _Kpi(
        Icons.workspace_premium_outlined,
        'Com garantia',
        data.servicosComGarantia.toDouble(),
        _whole,
      ),
      _Kpi(
        Icons.no_accounts_outlined,
        'Sem garantia',
        data.servicosSemGarantia.toDouble(),
        _whole,
      ),
      _Kpi(
        Icons.edit_note_rounded,
        'Valor alterável',
        data.servicosValorAlteravel.toDouble(),
        _whole,
      ),
      _Kpi(
        Icons.rule_folder_outlined,
        'Cadastro incompleto',
        data.servicosCadastroIncompleto.toDouble(),
        _whole,
      ),
    ];
  ...
  Widget _serieCard(
    String title,
    String subtitle,
    List<ServicoDashboardSerieItem> items,
  ) {
    final ThemeData theme = Theme.of(context);
    final WebThemeTokens tokens = WebThemeTokens.of(context);
    final List<ServicoDashboardSerieItem> visible =
        items
            .where((ServicoDashboardSerieItem item) => item.quantidade > 0)
            .take(5)
            .toList();
    final double maxValue = visible.fold<double>(
      0,
      (double max, ServicoDashboardSerieItem item) =>
          item.quantidade > max ? item.quantidade : max,
    );
              ...
              : Column(
                children:
                    visible.asMap().entries.map((
                      MapEntry<int, ServicoDashboardSerieItem> entry,
                    ) {
                      final ServicoDashboardSerieItem item = entry.value;
                      final double percent =
                          maxValue <= 0
                              ? 0
                              : (item.quantidade / maxValue)
                                  .clamp(0.0, 1.0)
                                  .toDouble();
                      return Padding(
  ...
  Widget _alerts(List<ServicoDashboardAlerta> alerts) {
    ...
    return SixWebSectionCard(
      ...
      child:
          alerts.isEmpty
              ? const SixWebNoData(
                text: 'Nenhum alerta operacional encontrado.',
              )
              : Column(
                children:
                    alerts
                        .map(
                          (ServicoDashboardAlerta alert) => _notice(
  ...
  Widget _attention(List<ServicoDashboardItem> items) => SixWebSectionCard(
    ...
    child:
        items.isEmpty
            ? const SixWebNoData(
              text: 'Nenhum serviço com pendência cadastral.',
            )
            : Column(
              children:
                  items
                      .map(
                        (ServicoDashboardItem item) =>
                            _serviceTile(item, showProblem: true),
                      )
                      .toList(),
            ),
  );
            ...
            : Column(
              children:
                  items
                      .map(
                        (ServicoDashboardItem item) =>
                            _serviceTile(item, showProblem: false),
                      )
                      .toList(),
            ),
  );
  ...
  Widget _serviceTile(ServicoDashboardItem item, {required bool showProblem}) {
    ...
    final WebThemeTokens tokens = WebThemeTokens.of(context);
    final String title =
        item.nome.trim().isEmpty ? 'Serviço sem nome' : item.nome.trim();
    return Container(
      ...
      child: Row(
        children: <Widget>[
          Icon(Icons.home_repair_service_outlined, color: tokens.info),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                ...
                Text(
                  '${item.categoria.trim().isEmpty ? 'Sem categoria' : item.categoria} • ${_warranty(item.tempoGarantia)} • ${item.podeAlterarValorNaHora ? 'Alterável' : 'Fixo'}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
          ...
          Column(
            ...
            children: <Widget>[
              ...
              Text(
                showProblem ? item.problema : _warranty(item.tempoGarantia),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: TextStyle(
                  color: showProblem ? tokens.warning : tokens.secondaryText,
                  fontSize: 11,
                  fontWeight: FontWeight.w800,
                ),
              ),
  ...
  String _warranty(String value) =>
      value.trim().isEmpty ? 'Sem garantia' : value.trim();
