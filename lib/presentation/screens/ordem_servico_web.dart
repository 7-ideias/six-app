...
class _OrdemServicoWebState extends State<OrdemServicoWeb> {
  ...
  final TextEditingController _orcamentoOrigemController =
      TextEditingController(text: 'ORC-2026-00127');
  ...
  final TextEditingController _assinanteController = TextEditingController(
    text: 'Marina Oliveira',
  );

  final List<String> _statusDisponiveis = <String>[
    'Aberta',
    'Em análise',
  ...
  final List<String> _prioridades = <String>[
    'Baixa',
    'Normal',
    'Alta',
    'Urgente',
  ];
  final List<String> _origens = <String>[
    'Balcão',
    'WhatsApp',
    'Instagram',
    'Site',
  ...
  final List<String> _tipos = <String>[
    'Assistência técnica',
  ...
  final List<Map<String, dynamic>> _checklist = <Map<String, dynamic>>[
    ...
    <String, dynamic>{
      ...
      'descricao': 'Touch responde corretamente em toda a área.',
      'ok': false,
    },
    <String, dynamic>{
  ...
  final List<Map<String, dynamic>> _timeline = <Map<String, dynamic>>[
    ...
    <String, dynamic>{
      ...
      'descricao': 'Hipótese principal validada e orçamento vinculado.',
      'hora': 'Hoje • 09:42',
  ...
  List<Map<String, dynamic>> get _steps => const <Map<String, dynamic>>[
    <String, dynamic>{
      'titulo': 'Abertura',
      'descricao': 'Status, origem e contexto',
      'icone': Icons.assignment_add,
    },
    <String, dynamic>{
      'titulo': 'Cliente e item',
      'descricao': 'Cadastro e check-in',
      'icone': Icons.devices_other_outlined,
    },
    <String, dynamic>{
      'titulo': 'Diagnóstico',
      'descricao': 'Defeito e checklist',
      'icone': Icons.medical_information_outlined,
    },
    <String, dynamic>{
      'titulo': 'Execução',
      'descricao': 'Técnico e tarefas',
      'icone': Icons.engineering_outlined,
    },
    <String, dynamic>{
      'titulo': 'Itens e custos',
      'descricao': 'Peças, serviços e aprovação',
      'icone': Icons.inventory_2_outlined,
    },
    <String, dynamic>{
      'titulo': 'Entrega',
      'descricao': 'Prazo, QR e assinatura',
  ...
  double _total() {
    return _itens
        .where((Map<String, dynamic> e) => e['selecionado'] == true)
        .fold<double>(
          0,
          (double total, Map<String, dynamic> e) =>
              total +
              (((e['valor'] ?? 0) as num).toDouble() *
                  ((e['quantidade'] ?? 1) as int)),
        );
  }
  ...
  int _qtd() => _itens
      ...
      .fold<int>(
        0,
        (int total, Map<String, dynamic> e) =>
            total + ((e['quantidade'] ?? 1) as int),
      );
  int _okCheck() =>
      _checklist.where((Map<String, dynamic> e) => e['ok'] == true).length;
  int _okTarefas() =>
      _tarefas.where((Map<String, dynamic> e) => e['ok'] == true).length;
  ...
  Future<void> _selecionarTecnico() async {
    final ColaboradorMock? colaborador = await showSelecaoColaboradorDialog(
      context,
      titulo: 'Selecionar técnico responsável',
    );
    if (colaborador == null) {
      return;
    }
  ...
  Future<void> _pickProduto() async {
    final ProdutoModel? result =
        await showProdutoListaSelecaoWebDialog<ProdutoModel>(
          context: context,
          widthFactor: 0.80,
          heightFactor: 0.80,
        );
    ...
    setState(() {
      ...
      _timeline.insert(0, <String, dynamic>{
        'titulo': 'Produto incluído na OS',
        'descricao':
            '${result.nomeProduto} foi adicionado ao fluxo da ordem de serviço.',
        'hora': 'Agora',
  ...
  Future<void> _signDialog() async {
    final TextEditingController nome = TextEditingController(
      text: _assinanteController.text,
    );
          ...
          builder: (_, StateSetter setModalState) {
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                TextButton.icon(
                  onPressed: () => setModalState(() => temp = <Offset?>[]),
                ...
                FilledButton.icon(
                  onPressed: () {
                    if (temp.whereType<Offset>().isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Faça a assinatura antes de salvar.'),
                        ),
                      );
                      return;
                    }
    ...
    if (ok == true) {
      setState(() {
        ...
        _timeline.insert(0, <String, dynamic>{
          'titulo': 'Assinatura digital coletada',
          'descricao': 'Cliente revisou o resumo da OS e assinou digitalmente.',
          'hora': 'Agora',
        ...
        child: Column(
          children: <Widget>[
            Row(
              children: <Widget>[
                Expanded(
                  child: _tf(_osController, 'Número da OS', Icons.tag_outlined),
                ),
                ...
                Expanded(
                  child: _dd(
                    ...
                    (String? value) => setState(
                      () => _statusSelecionado = value ?? _statusSelecionado,
                    ),
                  ),
                ),
                const SizedBox(width: 14),
              ...
              children: <Widget>[
                Expanded(
                  child: _dd(
                    ...
                    _origens,
                    (String? value) => setState(
                      () => _origemSelecionada = value ?? _origemSelecionada,
                    ),
                  ),
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: _dd(
                    ...
                    _tipos,
                    (String? value) => setState(
                      () => _tipoSelecionado = value ?? _tipoSelecionado,
                    ),
                  ),
                ),
              ],
          ...
          children: <Widget>[
            Wrap(
              ...
              children: <Widget>[
                FilledButton.icon(
                  ...
                  label: const Text('Selecionar cliente cadastrado'),
                ),
                OutlinedButton.icon(
                  onPressed:
                ...
                OutlinedButton.icon(
                  onPressed: _selecionarTecnico,
                  icon: const Icon(Icons.engineering_outlined),
                  label: const Text('Selecionar técnico responsável'),
                ),
                OutlinedButton.icon(
                  onPressed:
            ...
            Row(
              children: <Widget>[
                ...
                const SizedBox(width: 14),
                Expanded(
                  child: _tf(_emailController, 'E-mail', Icons.email_outlined),
                ),
              ],
            ),
            const SizedBox(height: 14),
            ...
            Wrap(
              ...
              children: <Widget>[
                FilledButton.icon(
                  ...
                  label: const Text('Selecionar técnico'),
                ),
                OutlinedButton.icon(
                  onPressed:
                      () => showSubPainelCadastroColaborador(
              ...
              OutlinedButton.icon(
                onPressed:
                    () => setState(() {
                      _itens.add(<String, dynamic>{
                        'tipo': 'servico',
                        'nome': 'Serviço adicional mock',
                        'detalhe': 'Serviço incluído manualmente na OS',
                        'quantidade': 1,
                        'valor': 120.0,
                        'selecionado': true,
                      });
                  ...
                  child: Row(
                    children: <Widget>[
                      ...
                      SizedBox(
                        width: 120,
                        child: Text(
                          'R\$ ${(value * quantity).toStringAsFixed(2)}',
            ...
            children: <Widget>[
              ...
              SizedBox(
                ...
                child: SwitchListTile(
                  value: _requerSinal,
                  onChanged:
                      (bool value) => setState(() => _requerSinal = value),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          const SizedBox(height: 8),
          Text(
            'O cliente pode abrir este resumo no celular, revisar as informações da OS e assinar digitalmente neste fluxo mockado.',
          ),
          const SizedBox(height: 16),
          LayoutBuilder(
            builder: (_, BoxConstraints constraints) {
                          ...
                          OutlinedButton.icon(
                            onPressed:
                                () => _showInfo(
                                  ...
                                  'Neste fluxo mockado, este link abrirá uma página pública com resumo da OS e aceite digital.',
                                ),
              ...
              if (compact) {
                return Column(
                  children: <Widget>[qr, const SizedBox(height: 12), info],
                );
              }
              return Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[qr, const SizedBox(width: 14), info],
              );
            },
          ),
        ],
            ...
            Container(
              ...
              child: Row(
                children: <Widget>[
                  ...
                  Expanded(
                    child: Text(
                      'Assinatura realizada por ${_assinanteController.text.trim().isEmpty ? 'cliente' : _assinanteController.text.trim()} em ${_dt(_assinadoEm)}.',
                      style: const TextStyle(fontWeight: FontWeight.w700),
                    ),
                  ),
                ],
        ...
        child: Column(
          children: <Widget>[
            ...
            Wrap(
              ...
              runSpacing: 12,
              children:
                  _canais.map((Map<String, dynamic> item) {
                    final bool selected = item['selecionado'] == true;
                    return FilterChip(
                      selected: selected,
                      avatar: Icon(
                      ...
                      onSelected:
                          (bool value) =>
                              setState(() => item['selecionado'] = value),
                    );
                  }).toList(),
            ),
            const SizedBox(height: 18),
              ...
              children: <Widget>[
                ...
                SizedBox(
                  ...
                  child: SwitchListTile(
                    ...
                    onChanged:
                        (bool value) =>
                            setState(() => _autorizaContato = value),
            ...
            children: <Widget>[
              ...
              OutlinedButton.icon(
                onPressed:
                    () => _showInfo(
                      'Salvar rascunho',
                      'Na próxima etapa você pode persistir a OS em backend sem concluir o fluxo.',
                    ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          context.t(
            'web.standalone.serviceOrder',
            fallback: 'Ordem de serviço',
          ),
        ),
      ),
      body: body,
          ...
          (int row) => Expanded(
            child: Row(
              children: List<Widget>.generate(
                ...
                (int col) => Expanded(
                  child: Container(
                    color: black(row, col) ? Colors.black : Colors.white,
                  ),
                ),
              ),
            ),
        ...
        child: GestureDetector(
          onPanStart:
              (DragStartDetails details) =>
                  onChanged(<Offset?>[...points, details.localPosition]),
          onPanUpdate:
