...
class AtendimentoTecnicoApiClient {
  ...
  Future<AtendimentoTecnicoDominiosBaseModel> buscarDominiosBase() async {
    ...
    if (response.statusCode != 200) {
      throw AtendimentoTecnicoApiException(statusCode: response.statusCode, body: _decodeBody(response));
    }
  ...
  Future<List<DominioStatusAtendimentoCustomizacaoModel>> salvarCustomizacoesStatusAtendimento(
  ...
  Future<Map<String, dynamic>> consultarAssinaturaPublica({required String idUnicoDaEmpresa, required String token}) async {
    final uri = Uri.parse('${AppConfig.baseUrl}/public/api/atendimentos-tecnicos/assinatura').replace(queryParameters: <String, String>{
