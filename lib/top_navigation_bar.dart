...
class TopNavItemData {
  const TopNavItemData({required this.title, required this.subItems, this.onSelect});
  final String title;
...
class TopNavigationBar extends StatelessWidget implements PreferredSizeWidget {
  const TopNavigationBar({super.key, required this.items, this.onNotificationPressed, this.notificationWidget});
  ...
  bool get _usaNovoMenuSix {
    ...
    return titles.contains('Cadastros') && titles.contains('Configurações') && titles.contains('Início');
  }
  ...
  void _mostrarPreparacao(BuildContext context, String label) {
    ...
    messenger?.showSnackBar(SnackBar(content: Text('$label: menu criado. A tela será evoluída nos próximos passos.'), behavior: SnackBarBehavior.floating));
  }

  Future<void> _abrirOverlay(BuildContext context, Widget Function(BuildContext dialogContext) builder, {double widthFactor = 0.94, double heightFactor = 0.90}) async {
      ...
      builder: (dialogContext) {
        ...
        return _EscOverlayScope(
          child: Dialog(
            ...
            child: SizedBox(width: size.width * widthFactor, height: size.height * heightFactor, child: builder(dialogContext)),
          ),
        );
      },
    );
  ...
  void _fecharEAbrirLegado(BuildContext dialogContext, BuildContext context, String title, String value) {
    Navigator.of(dialogContext).pop();
    Future<void>.delayed(const Duration(milliseconds: 80), () => _abrirLegado(context, title, value));
  }

  void _fecharEPreparar(BuildContext dialogContext, BuildContext context, String value) {
    Navigator.of(dialogContext).pop();
  ...
  List<TopNavItemData> _itemsEfetivos(BuildContext context) {
    ...
    return <TopNavItemData>[
      TopNavItemData(title: 'Início', subItems: const <String>[], onSelect: (_) => Navigator.of(context, rootNavigator: true).pushNamedAndRemoveUntil('/app', (_) => false)),
      TopNavItemData(title: 'Atendimento', subItems: const <String>['Nova venda', 'Atendimento técnico', 'Atendimentos criados', 'Novo orçamento', 'Nova assistência técnica', 'Vendas', 'Orçamentos', 'Assistências técnicas'], onSelect: (value) {
        if (value == 'Atendimento técnico' || value == 'Nova assistência técnica') return _abrirAtendimentoTecnico(context);
        if (value == 'Atendimentos criados' || value == 'Assistências técnicas') return _abrirAtendimentosCriados(context);
        _mostrarPreparacao(context, value);
      }),
      TopNavItemData(title: 'Catálogo', subItems: const <String>['Produtos', 'Serviços', 'Categorias', 'Estoque'], onSelect: (value) {
      ...
      TopNavItemData(title: 'Pessoas', subItems: const <String>['Clientes', 'Colaboradores', 'Fornecedores'], onSelect: (value) {
      ...
      TopNavItemData(title: 'Caixa', subItems: const <String>['Abrir caixa', 'Fechar caixa', 'Movimentações', 'Suprimento', 'Sangria', 'Retirada para despesa', 'Ajustes', 'Resumo do caixa'], onSelect: (value) => _mostrarPreparacao(context, value)),
      TopNavItemData(title: 'Financeiro', subItems: const <String>['Contas a receber', 'Contas a pagar', 'Recebimentos futuros', 'Fiado', 'Crediário', 'Agenda financeira'], onSelect: (value) {
        if (value == 'Agenda financeira') return _abrirAgenda(context);
        _mostrarPreparacao(context, value);
      }),
      TopNavItemData(title: 'Relatórios', subItems: const <String>['Vendas', 'Assistências', 'Caixa', 'Financeiro', 'Produtos', 'Clientes'], onSelect: (value) => _mostrarPreparacao(context, 'Relatório de $value')),
      TopNavItemData(title: 'Configurações', subItems: const <String>['Empresa', 'Usuários e permissões', 'Regionalização', 'Formas de recebimento', 'Regras operacionais', 'Notificações', 'Modelos de PDF', 'Integrações'], onSelect: (value) => _abrirConfiguracao(context, value)),
      TopNavItemData(title: 'Legado', subItems: const <String>['Meu Perfil', 'Clientes', 'Clientes List', 'Produtos', 'Categorias', 'Colaboradores', 'Colaboradores List', 'Fornecedores', 'Produtos List', 'Preferências do Six'], onSelect: (value) {
          ...
          children: <Widget>[
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Row(children: effectiveItems.map((item) => Padding(padding: const EdgeInsets.only(right: 4), child: _TopNavigationMenuItem(data: item, colorScheme: colorScheme))).toList()),
              ),
            ),
            const SizedBox(width: 12),
            _AppVersionPill(colorScheme: colorScheme),
            const SizedBox(width: 10),
            notificationWidget ?? IconButton(onPressed: onNotificationPressed, icon: Icon(Icons.notifications_none, color: colorScheme.onPrimary), tooltip: 'Notificações'),
          ],
        ),
      ),
    );
...
class _EscOverlayScope extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Shortcuts(
      ...
      child: Actions(actions: <Type, Action<Intent>>{_EscOverlayIntent: CallbackAction<_EscOverlayIntent>(onInvoke: (_) {Navigator.of(context).maybePop(); return null;})}, child: Focus(autofocus: true, child: child)),
    );
  }
}
...
class _AppVersionPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(height: 36, padding: const EdgeInsets.symmetric(horizontal: 12), decoration: BoxDecoration(color: colorScheme.surface, borderRadius: BorderRadius.circular(999)), alignment: Alignment.center, child: Text(label, style: TextStyle(color: colorScheme.primary, fontSize: 12, fontWeight: FontWeight.w800)));
  }
}
...
class _TopNavigationMenuItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (data.subItems.isEmpty) return _TopNavChip(label: data.title, hasMenu: false, colorScheme: colorScheme, onTap: () => data.onSelect?.call(data.title));
    return PopupMenuButton<String>(offset: const Offset(0, 44), onSelected: (value) => data.onSelect?.call(value), itemBuilder: (context) => data.subItems.map((subItem) => PopupMenuItem<String>(value: subItem, child: Text(subItem))).toList(), child: _TopNavChip(label: data.title, hasMenu: true, colorScheme: colorScheme));
  }
}

class _TopNavChip extends StatefulWidget {
  const _TopNavChip({required this.label, required this.hasMenu, required this.colorScheme, this.onTap});
  final String label;
  final bool hasMenu;
  final ColorScheme colorScheme;
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(color: _hovering ? widget.colorScheme.onPrimary.withOpacity(0.12) : Colors.transparent, borderRadius: BorderRadius.circular(8)),
          child: Row(mainAxisSize: MainAxisSize.min, children: <Widget>[Text(widget.label, style: TextStyle(color: widget.colorScheme.onPrimary, fontSize: 17, fontWeight: _hovering ? FontWeight.w800 : FontWeight.w700)), if (widget.hasMenu) Icon(Icons.keyboard_arrow_down_rounded, color: widget.colorScheme.onPrimary, size: 18)]),
        ),
      ),
    );
  }
